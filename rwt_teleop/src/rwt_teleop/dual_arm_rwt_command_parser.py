...
class DualArmRwtCommandParser(object):
    ...
    sgns = {"l": 1, "r": -1}
    node_name = "DualArmRwtCommandParser"
    ...
    def __init__(self):
        ...
        # subs
        self.clk_pt_sub = rospy.Subscriber(
            "/pointcloud_screenpoint_nodelet/output_point",
            PointStamped, self.clk_pt_cb)
        self.cmd_str_sub = rospy.Subscriber(
            "/rwt_command_string", String, self.cmd_str_cb)
        # pubs
        self.hp_pub = rospy.Publisher(
            '/master_head_pose', PoseStamped, queue_size=1)
        self.hp = PoseStamped(header=Header(frame_id=self.base_link_name))
        self.fb_str_pub = rospy.Publisher(
            '/rwt_current_state', String, queue_size=1)
        # l=left, r=right, d=dual
        self.lrd_for_clk = "r"
        ...
        self.task_context = {}
        for lr in self.LR:
            self.eew_sub[lr] = rospy.Subscriber(
                lr+"_wrench", WrenchStamped, self.eew, lr)
            self.eew[lr] = WrenchStamped
            self.eep_pub[lr] = rospy.Publisher(
                '/master_'+lr+'arm_pose', PoseStamped, queue_size=1)
            self.eep[lr] = PoseStamped(
                header=Header(frame_id=self.base_link_name),
                pose=Pose(
                    position=Point(*self.init_pose_xyz[lr]),
                    orientation=Quaternion_from_rpy(*self.init_pose_rpy[lr])
                )
            )
            # should be overrided
            self.g_pub[lr] = None
    ...
    def clk_pt_cb(self, msg):
        try:
            self.tfl.waitForTransform(
                "/"+self.base_link_name, "/rwt_clicked_point",
                msg.header.stamp, timeout=rospy.Duration(1))
            (pos, rot) = self.tfl.lookupTransform(
                "/"+self.base_link_name, "/rwt_clicked_point", rospy.Time(0))
        ...
        if self.lrd_for_clk == "d":
            ...
            for lr in self.LR:
                ...
                self.eep[lr].pose.orientation = Quaternion(
            ...
            for lr in lrd2lr_list(self.lrd_for_clk):
                ...
                if self.task_context[lr] == "table":
                    self.eep[lr].pose.position = Point(
    ...
    def cmd_str_cb(self, msg):
        ...
        if cmd == "resetpose":
            for lr in self.LR:
                self.eep[lr].pose.orientation = Quaternion_from_rpy(
                    *self.init_pose_rpy[lr])
        ...
        if tgt == "h" and cmd == "look":
            ...
            self.hp.pose.orientation = Quaternion(
                q[0], q[1], q[2], q[3])
        ...
        if tgt == "b" and cmd == "basepos":
            delta = 0.1
            if val == "+x":
                self.hp.pose.position.x += delta
            if val == "-x":
                self.hp.pose.position.x -= delta
            if val == "+y":
                self.hp.pose.position.y += delta
            if val == "-y":
                self.hp.pose.position.y -= delta
            if val == "+z":
                self.hp.pose.position.z += delta
            if val == "-z":
                self.hp.pose.position.z -= delta
            self.hp.pose.position.z = min(
                max(self.hp.pose.position.z, -0.2), 0.1)
            self.hp.header.stamp = rospy.Time.now()
            self.hp_pub.publish(self.hp)
        ...
        if cmd == "jogpos":
            ...
            for lr in lrd2lr_list(tgt):
                if val == "+x":
                    self.eep[lr].pose.position.x += delta
                if val == "-x":
                    self.eep[lr].pose.position.x -= delta
                if val == "+y":
                    self.eep[lr].pose.position.y += delta
                if val == "-y":
                    self.eep[lr].pose.position.y -= delta
                if val == "+z":
                    self.eep[lr].pose.position.z += delta
                if val == "-z":
                    self.eep[lr].pose.position.z -= delta
        ...
        if cmd == "jogrot":
            ...
            for lr in lrd2lr_list(tgt):
                if val == "+x":
                    q_rel = tf.transformations.quaternion_from_euler(
                        delta, 0, 0)
                if val == "-x":
                    q_rel = tf.transformations.quaternion_from_euler(
                        -delta, 0, 0)
                ...
                q_new = tf.transformations.quaternion_multiply(
                    q_rel, [q_org.x, q_org.y, q_org.z, q_org.w])
                self.eep[lr].pose.orientation = Quaternion(
            ...
            for lr in lrd2lr_list(tgt):
                ...
                for i in range(10):
                    f_norm = np.linalg.norm([
                        self.eew[lr].wrench.force.x,
                        self.eew[lr].wrench.force.y,
                        self.eew[lr].wrench.force.z
                        ]
                    )
                    if f_norm > float(val):
                        ...
                        if self.task_context[lr] == "table":
                            # for hanko demo
                            self.eep[lr].pose.position.z -= 0.005
                        else:
