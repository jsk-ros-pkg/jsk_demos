...
class DrawForceOnImage():
    ...
    def __init__(self):
        ...
        self.pub = {}
        for lr in self.LR:
            self.sub[lr] = rospy.Subscriber(
                lr+"_force_input", WrenchStamped, self.cb, lr)
            self.pub[lr] = rospy.Publisher(
                "/"+lr+"arm_marker/image_marker", ImageMarker2, queue_size=10)
    ...
    def cb(self, msg, lr):
        ...
        mrk.points3D.points = [
            Point(0, 0, 0),
            Point(msg.wrench.force.x / 50,
                  msg.wrench.force.y / 50,
                  msg.wrench.force.z / 50)
        ]
        raw_force_vec = np.array(
            [msg.wrench.force.x, msg.wrench.force.y, msg.wrench.force.z])
        # 20N = 100% warn = 1
        warn_level = np.clip(np.linalg.norm(raw_force_vec) / 50, 0, 1)
