...
class DualArmRwtCommandParser(object):
  ...
  def loop(self):
    ...
    rate = rospy.Rate(10) ## feedback message rate
  ...
  def cmd_str_cb(self, msg):
    ...
    if cmd == "tablemode":
      for lr in lrd2lr_list(tgt):
        ...
        self.eep[lr].pose.orientation = Quaternion_from_rpy(*self.table_pose_rpy[lr])
        self.eep[lr].header.stamp     = rospy.Time.now()
        self.eep_pub[lr].publish(self.eep[lr])
    ...
    if cmd == "frontmode":
      for lr in lrd2lr_list(tgt):
        ...
        self.eep[lr].pose.orientation = Quaternion_from_rpy(*self.front_pose_rpy[lr])
        self.eep[lr].header.stamp     = rospy.Time.now()
        self.eep_pub[lr].publish(self.eep[lr])
