        ...
        while True:
            ...
            if save_demo == 's':
                ...
                for f in os.listdir(self.save_dir_base):
                    if os.path.isfile(os.path.join(self.save_dir_base,f)):
                        ...
                        bagfile_name_con = join(self.target_dir, '{:03}_success_'.format(self.num_bagfiles) + f)
            ...
            if save_demo == 'f':
                ...
                for f in os.listdir(self.save_dir_base):
                    if os.path.isfile(os.path.join(self.save_dir_base,f)):
                        bagfile_name_origin = join(self.save_dir_base, f)
                        bagfile_name_con = join(self.target_dir, '{:03}_fault_'.format(self.num_bagfiles) + f)
                        cmd = "mv {} {}".format(bagfile_name_origin,bagfile_name_con)
                        cmd = shlex.split(cmd)
                        subprocess.call(cmd)
                break
...
main_script_path = "/home/amabe/franka_ws/src/jsk_demos/jsk_2021_fix_kxr/euslisp/cable-insertion_heuristic_insert.l"
