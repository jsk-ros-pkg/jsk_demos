...
class Listener:
    ...
    def log_state_and_speech(self, state_str, spoken_word):
        ...
        """
        log_path_default = os.path.expanduser(f"{self.path_to_pkg}/src/node_scripts/kashiwagi_speech_state_log.jsonl")
    ...
    def speech_callback(self, msg):
        ...
        #     self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_yotei_12_10.wav"
            ...
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_matane.wav"
        ...
        elif self.cur_kashiwagi_state in ["free_talk:listening_turn","free_talk:thinking_turn", "free_talk:speaking_turn"] and self.is_mentioned(spoken_word, ["おわり", "終わり", "ありがとう"]):
            self.during_speech_req_state = "daily:happy"
            self.after_speech_req_state = None
            ...
            self.during_speech_req_state = "daily:happy"
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_yoroshiku.wav"

        elif self.cur_kashiwagi_state == "daily:normal" and self.is_mentioned(spoken_word, ["すごい", "人気"]):
            self.during_speech_req_state = "daily:happy"
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_sorehododemo.wav"

        elif self.cur_kashiwagi_state == "daily:normal" and self.is_mentioned(spoken_word, ["やっほー", "ヤッホー"]):
            self.during_speech_req_state = "daily:happy"
            self.after_speech_req_state = None
        ...
        # elif self.cur_kashiwagi_state == "daily:normal" and self.is_mentioned(spoken_word, ["金額", "見学"]):
        #     self.during_speech_req_state = "daily:happy"
        #     self.after_speech_req_state = None
        #     self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_kitekuretano.wav"
...
#################
        ...
        elif self.cur_kashiwagi_state == "talking_game:listening_turn" and self.is_mentioned(spoken_word, ["やっほー", "ヤッホー"]):
            self.during_speech_req_state = "talking_game:happy"
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_yahho.wav"
...
############
        ...
        elif self.cur_kashiwagi_state == "katakanashi:playing" and self.is_mentioned(spoken_word, ["やっほー", "ヤッホー"]):
            self.during_speech_req_state = "katakanashi:happy"
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_yahho.wav"
...
###########
            ...
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_tanoshikattane.wav"
            with open(f"{self.path_to_pkg}/src/node_scripts/latest_shiritori_reply.txt", "w", encoding="utf-8") as f:
                f.write("")
        ...
        elif self.cur_kashiwagi_state == "shiritori:listening_turn" and self.is_mentioned(spoken_word, ["やっほー", "ヤッホー"]):
            self.during_speech_req_state = "shiritori:happy"
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_yahho.wav"
...
###########            
            ...
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_oyasumi.wav"
            ...
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_darekayonda.wav"
            ...
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_bikkuri.wav"
            ...
            self.during_speech_req_state = None
            self.after_speech_req_state = "move:finding_person"
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_dokodaro.wav"
        ...
        elif self.cur_kashiwagi_state == "move:staying" and self.is_mentioned(spoken_word, ["やっほー", "ヤッホー"]):
            self.during_speech_req_state = "move:happy"
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_yahho.wav"
...
#####################
            ...
            self.after_speech_req_state = "daily:happy"
            song = f"{self.path_to_pkg}/data/kashiwagi_song_donguri.mp3"
            self.sound_file = song

        elif self.cur_kashiwagi_state == "daily:normal" and self.is_mentioned(spoken_word, ["雪やこんこん"]):
            self.during_speech_req_state = "daily:singing"
            self.after_speech_req_state = "daily:happy"
            song = f"{self.path_to_pkg}/data/kashiwagi_song_yuki.mp3"
            self.sound_file = song

        elif self.cur_kashiwagi_state == "daily:normal" and self.is_mentioned(spoken_word, ["でんでん"]):
            self.during_speech_req_state = "daily:singing"
            self.after_speech_req_state = "daily:happy"
            song = f"{self.path_to_pkg}/data/kashiwagi_song_katatsumuri.mp3"
            self.sound_file = song

        elif self.cur_kashiwagi_state == "daily:normal" and self.is_mentioned(spoken_word, ["桃太郎"]):
            self.during_speech_req_state = "daily:singing"
            self.after_speech_req_state = "daily:happy"
            song = f"{self.path_to_pkg}/data/kashiwagi_song_momotaro.mp3"
            self.sound_file = song

        elif self.cur_kashiwagi_state == "daily:normal" and self.is_mentioned(spoken_word, ["アイドル"]):
            self.during_speech_req_state = "daily:singing"
            self.after_speech_req_state = "daily:happy"
            song = f"{self.path_to_pkg}/data/kashiwagi_song_idle.mp3"
            self.sound_file = song

        elif self.cur_kashiwagi_state == "daily:normal" and self.is_mentioned(spoken_word, ["仰げば尊"]):
            self.during_speech_req_state = "daily:singing"
            self.after_speech_req_state = "daily:happy"
            song = f"{self.path_to_pkg}/data/kashiwagi_song_aogeba.mp3"
            self.sound_file = song
            
        elif self.cur_kashiwagi_state == "daily:normal" and self.is_mentioned(spoken_word, ["ぽっぽ"]):
            self.during_speech_req_state = "daily:singing"
            self.after_speech_req_state = "daily:happy"
            song = f"{self.path_to_pkg}/data/kashiwagi_song_hato.mp3"
            self.sound_file = song

        elif self.cur_kashiwagi_state == "daily:normal" and self.is_mentioned(spoken_word, ["君が代"]):
            self.during_speech_req_state = "daily:singing"
            self.after_speech_req_state = "daily:happy"
            song = f"{self.path_to_pkg}/data/kashiwagi_song_kimigayo.mp3"
            self.sound_file = song

        elif self.cur_kashiwagi_state == "daily:normal" and self.is_mentioned(spoken_word, ["勇者"]):
            self.during_speech_req_state = "daily:singing"
            self.after_speech_req_state = "daily:happy"
            ...
            self.during_speech_req_state = "daily:singing"
            self.after_speech_req_state = "daily:happy"
            songs = [f"{self.path_to_pkg}/data/kashiwagi_song_yuki.mp3",
                     f"{self.path_to_pkg}/data/kashiwagi_song_donguri.mp3",
                     f"{self.path_to_pkg}/data/kashiwagi_song_katatsumuri.mp3",
                     f"{self.path_to_pkg}/data/kashiwagi_song_momotaro.mp3",
                     f"{self.path_to_pkg}/data/kashiwagi_song_hato.mp3",
                     f"{self.path_to_pkg}/data/kashiwagi_song_aogeba.mp3",
                     f"{self.path_to_pkg}/data/kashiwagi_song_sakura.mp3",
        ...
        elif self.cur_kashiwagi_state == "daily:normal" and self.is_mentioned(spoken_word, ["声変えて", "違う声"]):
            self.during_speech_req_state = "daily:singing"
            self.after_speech_req_state = "daily:happy"
            songs = [
                f"{self.path_to_pkg}/data/kashiwagi_voice_change{i}.{ext}"
                for i in range(1, 7)
                for ext in ["wav", "mp3"]]
            self.sound_file = random.choice(songs)
        
        elif self.cur_kashiwagi_state == "talking_game:listening_turn" and self.is_mentioned(spoken_word, ["柏木さん", "柏", "柏木", "押上", "押上さん", "西脇さん", "西脇", "芦屋駅", "うさぎさん"]):
            self.during_speech_req_state = "talking_game:happy"
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_yonda.wav"

        elif self.cur_kashiwagi_state == "katakanashi:playing" and self.is_mentioned(spoken_word, ["柏木さん", "柏", "柏木", "押上", "押上さん", "西脇さん", "西脇", "芦屋駅", "うさぎさん"]):
            self.during_speech_req_state = "katakanashi:happy"
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_yonda.wav"

        elif self.cur_kashiwagi_state == "shiritori:listening_turn" and self.is_mentioned(spoken_word, ["柏木さん", "柏", "柏木", "押上", "押上さん", "西脇さん", "西脇", "芦屋駅", "うさぎさん"]):
            self.during_speech_req_state = "shiritori:happy"
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_yonda.wav"

        elif self.cur_kashiwagi_state == "daily:normal" and self.is_mentioned(spoken_word, ["柏木さん", "柏", "柏木", "押上", "押上さん", "西脇さん", "西脇", "芦屋駅", "うさぎさん", "未来"]):
            self.during_speech_req_state = "daily:happy"
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_yonda.wav"

        elif self.cur_kashiwagi_state == "talking_game:listening_turn" and self.is_mentioned(spoken_word, ["ピッピ"]):
            self.during_speech_req_state = "talking_game:happy"
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_pippidayo.wav"

        elif self.cur_kashiwagi_state == "katakanashi:playing" and self.is_mentioned(spoken_word, ["ピッピ"]):
            self.during_speech_req_state = "katakanashi:happy"
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_pippidayo.wav"

        elif self.cur_kashiwagi_state == "shiritori:listening_turn" and self.is_mentioned(spoken_word, ["ピッピ"]):
            self.during_speech_req_state = "shiritori:happy"
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_pippidayo.wav"

        elif self.cur_kashiwagi_state == "daily:normal" and self.is_mentioned(spoken_word, ["ピッピ"]):
            self.during_speech_req_state = "daily:happy"
            self.after_speech_req_state = None
            self.sound_file = f"{self.path_to_pkg}/data/kashiwagi_pippidayo.wav"
