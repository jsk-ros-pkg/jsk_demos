...
class ResponseGenerator:
    def __init__(self):
        ...
        self.system_prompt = {
            ...
            "content": (
                ...
                "見た目は鳩で、柏の葉の羽と梅の飾りと背中に背負っている小さな黒色のリュックが特徴。首から下げている名札もお気に入りです"
    ...
    def build_input_text(self, prompt):
        """
        Responses API に渡すため、system_prompt と user prompt を1つの文字列にまとめる。
        """
        return (
            "システム指示:\n"
            f"{self.system_prompt['content']}\n\n"
            "ユーザー指示:\n"
            f"{prompt}"
        )
    ...
    def response_callback(self, msg):
        ...
        # change kashiwagi state to "talking_game:thinking_turn"
        try:
        ...
        if qr_text in self.qa_map:
            ...
            - 文字カウントをして200文字に収まっているのかを確認し、文章を途中で終わらせないでください。
            """

            print(prompt)

            input_text = self.build_input_text(prompt)
